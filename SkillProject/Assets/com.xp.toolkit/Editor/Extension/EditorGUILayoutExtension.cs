...
namespace XPToolchains.Extension
{
    public static partial class EditorGUILayoutExtension
    {
        ...
        {
            return ((!_fieldInfo.IsPrivate && !_fieldInfo.IsFamily) || AttributeHelper.TryGetFieldInfoAttribute(_fieldInfo, out SerializeField serAtt))
                    && !AttributeHelper.TryGetTypeAttribute(_fieldInfo.DeclaringType, out NonSerializedAttribute nonAtt)
                    && !AttributeHelper.TryGetFieldInfoAttribute(_fieldInfo, out HideInInspector hideAtt);
        }
        ...
        public static object DrawField(GUIContent _content, FieldInfo _fieldInfo, object _value)
        {
            if (typeof(IList).IsAssignableFrom(_fieldInfo.FieldType))
                return DrawArrayField(_content, _fieldInfo, _fieldInfo.FieldType, _value);
        ...
        public static object DrawField(FieldInfo _fieldInfo, object _value)
        {
            ...
            if (AttributeHelper.TryGetFieldInfoAttribute(_fieldInfo, out TooltipAttribute tooltipAtt))
        ...
        private static object DrawFields(object _object)
        {
            ...
            foreach (var field in ReflectionHelper.GetFieldInfos(_object.GetType()))
            {
                if (CanDraw(field))
                {
